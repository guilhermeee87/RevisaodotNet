using System;

namespace RevisaodotNet
{
    class Program
    {
        static void Main(string[] args)
        {   //possiveis opções
            Aluno[] alunos = new Aluno [5];
            var indiceAluno = 0;
            string opcaoUsuario = ObterOpcaoUsuario();

            while (opcaoUsuario != "4")
            {
                switch (opcaoUsuario)
                {
                    case "1":
                        //add aluno
                        Console.WriteLine("Nome Completo: ");
                        var aluno = new Aluno();
                        aluno.Nome = Console.ReadLine();

                        Console.WriteLine();
                        Console.WriteLine("Nota do Aluno:");

                       if (decimal.TryParse(Console.ReadLine(), out decimal nota))
                       {
                            aluno.Nota = nota;
                       }
                       else
                       {
                           throw new ArgumentException("Necessita ser decimal!");
                       }

                       alunos[indiceAluno] = aluno;
                       indiceAluno++;                        
                    
                        break;
                    case "2":
                        //list aluno
                        foreach(var a in alunos)
                        {
                            if (!string.IsNullOrEmpty(a.Nome))
                            {
                                Console.WriteLine($"ALUNO: {a.Nome} - NOTA {a.Nota}");
                            }
                            
                        }
                        break;
                    case "3":
                        //info media
                        decimal notaTotal = 0;
                        var nrAlunos = 0;

                        for (int i=0; i < alunos.Length; i++)
                        {
                            if (!string.IsNullOrEmpty(alunos[i].Nome))
                            {
                                notaTotal = notaTotal+alunos[i].Nota;
                                nrAlunos++;
                            }
                        }
                        var mediaGeral = notaTotal/nrAlunos; 
                        ConceitoE conceitoGeral;

                        if(mediaGeral < 2)
                        {
                            conceitoGeral = ConceitoE.E;
                        }
                        else if (mediaGeral < 4)
                        {
                            conceitoGeral = ConceitoE.D;
                        }
                        else if (mediaGeral < 6)
                        {
                            conceitoGeral = ConceitoE.C;
                        }
                        else if (mediaGeral < 8)
                        {
                            conceitoGeral = ConceitoE.B;
                        }
                        else
                        {
                            conceitoGeral = ConceitoE.A;
                        }
                        Console.WriteLine($"Média Geral: {mediaGeral.ToString("F")} - Conceito: {conceitoGeral}");//.ToString("F") para 2 casasa decimais
                        break;
                    default:
                        throw new ArgumentOutOfRangeException("Opção Invalída");

                }
                opcaoUsuario = ObterOpcaoUsuario();
            }
        }

        private static string ObterOpcaoUsuario()
        {
            Console.WriteLine();
            Console.WriteLine("Informe a sua opção");
            Console.WriteLine("1- Novo Aluno");
            Console.WriteLine("2- Listar Alunos");
            Console.WriteLine("3- Média geral");
            Console.WriteLine("4- Sair");
            Console.WriteLine();

            string opcaoUsuario = Console.ReadLine();
            Console.WriteLine("");
            return opcaoUsuario;
        }
    }
}
